...
@onready var game_world: GameWorld3D = get_tree().get_root().get_node_or_null("Main/GameWorld3D")
...
@onready var plot_debug_overlay: PlotDebugOverlay = $PlotDebugOverlay
...
func _show_login_menu() -> void:
	...
	if plot_debug_overlay != null:
		plot_debug_overlay.clear_panel()
...
func _ready() -> void:
	...
	if orders_menu_panel != null:
		orders_menu_panel.menu_action_requested.connect(_on_orders_menu_action_requested)
		orders_menu_panel.cancel_active_order_requested.connect(_on_orders_menu_cancel_active_order_requested)
...
func _unhandled_input(event: InputEvent) -> void:
	# F3 toggles the developer-only plot debug overlay.
	# This stays out of normal player UI flow and can be turned on only when
...
func _process(_delta: float) -> void:
	...
	if plot_debug_overlay != null and plot_debug_overlay.is_debug_enabled():
		_refresh_plot_debug_overlay()
...
func _toggle_plot_debug_overlay() -> void:
	if plot_debug_overlay == null:
		return

	var next_enabled: bool = not plot_debug_overlay.is_debug_enabled()
	plot_debug_overlay.set_debug_enabled(next_enabled)
...
func _refresh_plot_debug_overlay() -> void:
	if plot_debug_overlay == null:
		return

	if not plot_debug_overlay.is_debug_enabled():
		return
	...
	plot_debug_overlay.show_plot_debug(plot_id, detail)
...
func _refresh_orders_menu() -> void:
	...
	var active_order_text: String = PlotOrderDefinitions.build_active_order_text(detail)
	var order_entries: Array[PlotOrderMenuEntry] = PlotOrderDefinitions.build_entries(detail)
...
func _request_plot_order(order_kind: String, target_scope: String) -> void:
	...
	if orders_menu_panel != null:
		orders_menu_panel.set_request_pending("Issuing order...")
...
func _request_cancel_active_order() -> void:
	...
	if orders_menu_panel != null:
		orders_menu_panel.set_request_pending("Cancelling active order...")

	net.cancel_plot_order(_active_player_plot_id)
	_set_status_text("Cancelling active order...")
...
func _on_orders_menu_action_requested(
	action_type: String,
	order_kind: String,
	target_scope: String
) -> void:
	match action_type:
		PlotOrderDefinitions.ACTION_TYPE_CANCEL_ACTIVE_ORDER:
...
func _on_orders_menu_cancel_active_order_requested() -> void:
	_request_cancel_active_order()
...
func _on_cancel_plot_order_result(result: Dictionary) -> void:
	var was_success: bool = bool(result.get("ok", false))

	if was_success:
		var cancelled_order_kind: String = str(result.get("cancelled_order_kind", ""))
		var success_text: String = "Active order cancelled."

		if cancelled_order_kind == PlotOrderDefinitions.ORDER_KIND_SCAVENGING:
